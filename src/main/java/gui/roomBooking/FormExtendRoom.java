...
public class FormExtendRoom extends JDialog {
    ...
    private void initUI() {
        ...
        LocalDateTime[] times = roomBUS.getActiveStayTimes(roomID);
        LocalDateTime checkIn  = times[0];
        LocalDateTime oldCheckOut   = times[1];

        txtOldOut = roText(oldCheckOut == null ? "(không có)" : oldCheckOut.format(formatter));

        txtNewOut = text();
        txtNewOut.setToolTipText("Nhập theo định dạng: dd-MM-yyyy HH:mm"); // rê chuột vào de hien thi
        if (oldCheckOut != null) txtNewOut.setText(oldCheckOut.plusHours(1).format(formatter));  // gợi ý mặc định
        ...
        btnApply.addActionListener(e -> ExtendRoom(checkIn, oldCheckOut));
    }

    private void ExtendRoom(LocalDateTime checkIn, LocalDateTime oldOut) {
        String sNew = txtNewOut.getText().trim();
        if (sNew.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Vui lòng nhập thời gian check-out mới.");
            return;
        }

        LocalDateTime newCheckOutDate;
        try {
            newCheckOutDate = LocalDateTime.parse(sNew, formatter);
        }
        catch (Exception ex) {
            JOptionPane.showMessageDialog(this, "Định dạng phải là dd-MM-yyyy HH:mm");
            return;
        }
        ...
        boolean ok = roomBUS.giaHanPhong(roomID, newCheckOutDate);
        if (ok) {
            ...
            JOptionPane.showMessageDialog(this,
                    roomBUS.getLastError() == null ? "Không thể gia hạn." : roomBUS.getLastError(),
