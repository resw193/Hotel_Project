...
public class FormRoomBookingManagement extends JPanel {
    ...
    private void loadData() {
        ...
        ArrayList<Room> dsPhong = switch (status) {
            case "Trống" -> roomDAO.getAllRoomOccupancy(1);
            case "Đặt"   -> roomDAO.getAllRoomByStatus("Đặt");
            case "Check-in" -> roomDAO.getAllRoomByStatus("Check-in");
            default -> roomDAO.getAllRoomOccupancy(1);
        };
    ...
    private JComponent createRoomCard(Room room) {
        ...
        try {
            ImageIcon ic = new ImageIcon(room.getImgRoomSource());
            if (ic.getImageLoadStatus() != MediaTracker.COMPLETE)
                ic = new ImageIcon("images/404-not-found.jpg");
            Image scaled = ic.getImage().getScaledInstance(180, -1, Image.SCALE_SMOOTH);
            img.setIcon(new ImageIcon(scaled));
        } catch (Exception ex) {
            img.setIcon(new ImageIcon("images/404-not-found.jpg"));
        }
        ...
        miBookRoom.addActionListener(e -> {
            String empID = accountDAO.getAccountByUsername(Application.username).getEmployee().getEmployeeID();
        ...
        miCheckIn.addActionListener(e -> {
            ...
            if (opt == JOptionPane.YES_OPTION) {
                boolean ok = new RoomDAO().checkIn(room.getRoomID());
                JOptionPane.showMessageDialog(this, ok ? "Đã Check-in" : "Không thể Check-in");
                loadData();
            }
        });
        ...
        miCheckOut.addActionListener(e -> {
            ...
            if (opt == JOptionPane.YES_OPTION) {
                boolean ok = new RoomDAO().checkOut(room.getRoomID());
                JOptionPane.showMessageDialog(this, ok ? "Đã Check-out" : "Không thể Check-out");
                loadData();
            }
        });
