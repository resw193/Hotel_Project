...
public class FormCancelBooking extends JPanel {
    ...
    private void loadData() {
        ...
        dsPhong = roomDAO.getAllRoomByStatus("Đặt");
    ...
    private JComponent createRoomCard(Room room) {
        ...
        try {
            ...
            Image scaled = ic.getImage().getScaledInstance(220, -1, Image.SCALE_SMOOTH);
        ...
        btnCancel.addActionListener(e -> {
            ...
            if (opt == JOptionPane.YES_OPTION) {
                boolean ok = new RoomDAO().huyDatPhong(room.getRoomID());
                JOptionPane.showMessageDialog(this, ok ? "Đã hủy đặt phòng" : "Không thể hủy đặt phòng");
                loadData();
            }
        });
    ...
    private void onCancel(String roomID) {
        int opt = JOptionPane.showConfirmDialog(
                this,
                "Xác nhận hủy đặt phòng " + roomID + " ?",
                "Xác nhận",
                JOptionPane.YES_NO_OPTION
        );
        if (opt == JOptionPane.YES_OPTION) {
            boolean ok = roomDAO.huyDatPhong(roomID);
            JOptionPane.showMessageDialog(this, ok ? "Đã hủy đặt phòng." : "Không thể hủy đặt phòng.");
            loadData();
        }
    }
