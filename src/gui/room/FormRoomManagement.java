...
public class FormRoomManagement extends JPanel {
    ...
    public void searchAndFilter() {
        ...
        ArrayList<Room> data = "Check-in".equals(filter)
                ? roomDAO.getAllRoomByStatus("Check-in")
                : roomDAO.getAllRoom();

        if (!keyword.isEmpty()) {
            String kw = keyword.toLowerCase();
            data = data.stream()
                    .filter(room -> room.getRoomID() != null && room.getRoomID().toLowerCase().contains(kw))
                    .collect(Collectors.toCollection(ArrayList::new));
        }
        roomTableModel.setRooms(data);
    }
}
